...
import { defaultCropRects, finalizeSnapshot, loadOwnedSixGrid, sourceForCrop } from '@/lib/novel-promotion/six-grid/image-task-route'
...
const schema = z.object({ episodeId: z.string().trim().min(1).max(200), storyboardId: z.string().trim().min(1).max(200),
  cropRects: z.array(z.object({ cellIndex: z.number().int().min(0).max(5), normalizedCropRect: rect }).strict()).length(6).optional(),
...
export const POST = apiHandler(async (request: NextRequest, context: { params: Promise<{ projectId: string }> }) => {
  ...
  const { order, media } = sourceForCrop(storyboard)
  const cropRects = body.cropRects || defaultCropRects()
  if (new Set(cropRects.map((item) => item.cellIndex)).size !== 6) throw new ApiError('INVALID_PARAMS', { code: 'SIX_GRID_CROP_INDEXES_INVALID', field: 'cropRects' })
