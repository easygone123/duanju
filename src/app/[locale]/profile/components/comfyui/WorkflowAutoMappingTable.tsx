...
export default function WorkflowAutoMappingTable({ analysis, roles, primaryOutputNodeId, disabled = false, onRoleChange, onPrimaryOutputChange }: WorkflowAutoMappingTableProps) {
  ...
  return <div className="min-w-0 space-y-5">
    <section className="space-y-2" aria-labelledby="automatic-input-mappings">
      ...
      <div className="space-y-2">{analysis.proposals.map((proposal) => {
        ...
        const selected = roles[proposal.id]
          || (confidence === 'ambiguous'
            ? proposal.required ? '' : 'preserve_original'
            : confidence === 'preserve_original' ? 'preserve_original' : proposal.canonicalName)
        return <div key={proposal.id} className="grid min-w-0 gap-2 rounded-xl border border-[var(--glass-stroke-base)] p-3 sm:grid-cols-2">
          ...
          <label className="text-xs">{t('mappedRole')}<select disabled={disabled} value={selected} onChange={(event) => onRoleChange(proposal.id, event.target.value as CanonicalWorkflowInput | 'preserve_original')} className="mt-1 w-full min-w-0 rounded-lg border border-[var(--glass-stroke-base)] bg-[var(--glass-bg-surface)] px-2 py-2">
            {confidence === 'ambiguous' && proposal.required && <option value="">{t('selectMappingRole')}</option>}
    ...
    <section className="space-y-2" aria-labelledby="automatic-output-mappings"><h4 id="automatic-output-mappings" className="font-medium">{t('outputMappings')}</h4>
      {analysis.outputs.length === 0 ? <p role="alert" className="rounded-lg border border-[var(--glass-stroke-danger)] p-3 text-sm text-[var(--glass-tone-danger-fg)]">{t('primaryOutputMissing')}</p> : analysis.outputs.map((output) => <label key={output.nodeId} className="flex min-w-0 items-center gap-2 rounded-lg border border-[var(--glass-stroke-base)] p-3 text-sm"><input type="radio" name="automatic-primary-output" disabled={disabled} checked={primaryOutputNodeId === output.nodeId} onChange={() => onPrimaryOutputChange(output.nodeId)} /><span className="min-w-0 break-all">{output.name} · {output.nodeId}.{output.fieldPath}</span></label>)}
    </section>
  </div>
}
