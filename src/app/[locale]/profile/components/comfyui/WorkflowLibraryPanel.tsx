...
import { useComfyConnections, useComfyStatuses } from './hooks'
import {
  draftFromWorkflow,
  emptyWorkflowDraft,
...
type ErrorKey = 'requestFailed' | 'workflowInvalidJson' | 'workflowTooLarge'
...
export default function WorkflowLibraryPanel() {
  ...
  const statusesQuery = useComfyStatuses(true)
  ...
  const testVersion = () => runAction(async () => {
    ...
    await requestJson(`/api/comfyui/workflows/${encodeURIComponent(selectedId)}/test-run`, {
      method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ versionId: savedVersion.id, connectionId, variables: {}, uploads: {} }),
    })
    await load(selectedId)
  })

  const statusById = useMemo(() => new Map((statusesQuery.data?.statuses ?? []).map((status) => [status.connectionId, status])), [statusesQuery.data])
  const compatibility: WorkflowCompatibilityView[] = (connectionsQuery.data?.connections ?? []).map((connection) => {
    const state = statusById.get(connection.id)?.state
    return {
      connectionId: connection.id,
      connectionName: connection.name,
      state: state === 'workflow_incompatible' ? 'incompatible' : state === 'offline' || state === 'auth_failed' ? 'offline' : state ? 'compatible' : 'unknown',
  ...
  return <section aria-labelledby="comfyui-workflow-library-heading" className="flex min-h-0 flex-col overflow-hidden">
    ...
    <div className="grid min-h-0 flex-1 gap-5 overflow-y-auto p-5 lg:grid-cols-[16rem_minmax(0,1fr)] sm:p-6">
      ...
      <div className="min-w-0 space-y-6">
        <WorkflowEditor value={authorDraft} disabled={busy} onChange={setAuthorDraft} onImportError={(key) => setError(key as ErrorKey)} />
        ...
        <div className="flex flex-wrap items-end gap-2">
          ...
          <button type="button" onClick={() => void testVersion()} disabled={busy || !savedVersion || !connectionId} className="glass-btn-base px-4 py-2 text-sm">{t('test')}</button>
        </div>
