...
export function useWorkspaceProjectSnapshot({
  ...
  return useMemo(() => {
    ...
    return {
      ...
      storyboardGenerationMode: projectData?.storyboardGenerationMode || 'individual',
      sixGridCellAspectRatio: projectData?.sixGridCellAspectRatio || null,
      sixGridProcessingOrder: projectData?.sixGridProcessingOrder || 'crop_then_panel_upscale',
      storyboardUpscaleModel: projectData?.storyboardUpscaleModel || null,
      dialogueVideoModel: projectData?.dialogueVideoModel || null,
      capabilityOverrides,
